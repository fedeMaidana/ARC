...
fn classifies_enabled_registered_source() {
    let config = agents_config(false, vec![agent_source("example-agent", true)]);

    let source = classify_source("example-agent", &config).expect("source should be allowed");

    assert_eq!(source.id(), "example-agent");
...
fn rejects_disabled_registered_source() {
    let config = agents_config(true, vec![agent_source("example-agent", false)]);

    let error = classify_source("example-agent", &config).expect_err("source should be rejected");

    match error {
        AgentSourceError::Disabled { source_id } => {
            assert_eq!(source_id, "example-agent");
        }
...
fn allows_unknown_source_when_unknown_sources_are_allowed() {
    let config = agents_config(true, vec![agent_source("example-agent", true)]);
